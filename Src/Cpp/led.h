...
#define LED_RED_MODE_BIT (1U<<28)       // Flyttar en bit 28 steg vänster till MODER14[1:0]
#define LED_GREEN_MODE_BIT (1U<<24)     // som ovan resp 24 vänster
...
typedef enum {   
  ...
  YELLOW,
  BLUE
}LedColor_Type;     // Typedef som skickar ut färg på LED

//Definerar ju / numrerar de olika lägena som LED kan befinna sig i
typedef enum {
  OFF = 0,
  ON = 1
}LedState_Type;   // Typedef som skickar ut tillstånd/state på LED

// Definerar de attribut som LED består utav
class Led{                // En klass för att skapa Led objekt. 

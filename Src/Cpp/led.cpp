...
void Led::setState(LedState_Type _state){
  ...
  switch(this->color){
    ...
    case RED:
      ...
      LED_PORT->MODER |= LED_RED_MODE_BIT;  // Mode/PB 14 till [1:0] enligt led.h
      //Om önskad status är ON
      if(this->state == ON){                // Pekare till enum ON enligt led.h
        //Definerar pinensouput till aktiv
        LED_PORT->ODR |= LED_RED_PIN;       // Sätter på RED genom Output Data Register
      }
      else{
        //Definerar pin output till inaktiv
        LED_PORT->ODR &= ~LED_RED_PIN;      // Växlar biten till 0 genom ODR
      }
      break;
